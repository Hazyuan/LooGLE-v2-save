...
def get_pred(dataset, args, fout):
    ...
    for data in tqdm(dataset):
        try:
            data["context"] = " ".join(data["context_rag"][:args.topk])
        except:
            print("Too short")
            pass
...
def main():
    ...
    out_file = os.path.join(args.save_dir, f"{args.model}_{str(args.topk)}.jsonl")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", "-d", type=str, default="datasets/dataset/splits/test_sample_rag_filtered.jsonl")
    parser.add_argument("--topk", "-k", type=int, default=16)
